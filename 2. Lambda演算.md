...
3. 如果我们具有一个值 f，f的值为“λx.x+1”，那么表达式“f 1”或“(λx.x+1) 1”的值就相当于将1带入x后进行计算，类似数学上从“f(x) = x + 1”带入“x = 1”后得到“f(1) = 1 + 1”，最终得到的结果为2，此结果也是一个值。
...
let mapBoolean = fun v ->		// 这个函数用来把F#中的true和false变换成上面λ演算表达式中的true函数和false函数
	if v = true then (fun a b -> a)	// 这里会产生警告“'b' is unused.”，可以使用_替换掉b，“_”的意思是不使用这个参数。
	else (fun a b -> b)
...
1. 将if和then之间的表达式进行求值，得到true或false，并替换成fun a b -> a和fun a b -> b。
